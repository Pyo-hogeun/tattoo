  ...
  <div class="mx-auto flex max-w-7xl gap-6 p-6">
    ...
    <section class="flex-1 rounded-xl border bg-white p-4 shadow-sm">
      ...
      <form class="grid gap-2 md:grid-cols-2" @submit.prevent="submit">
        ...
        <input v-model="form.imageUrl" class="rounded border p-2" placeholder="이미지 URL (또는 아래 파일 업로드)" />
        <input class="rounded border p-2 md:col-span-2" type="file" accept="image/*" @change="onImageFileChange" />
        <img v-if="previewImage" :src="previewImage" class="h-24 w-36 rounded border object-cover md:col-span-2" />
      ...
      <div class="mt-6">
        ...
        <div class="space-y-2">
          <div v-for="item in store.browShapes" :key="item._id" class="flex items-center gap-3 rounded border p-2">
            <img :src="resolveImageUrl(item.imageUrl)" :alt="item.name" class="h-16 w-24 rounded object-cover" />
            <div class="flex-1">
