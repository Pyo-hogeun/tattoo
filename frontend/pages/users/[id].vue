...
const fetchUser = async () => {
  ...
  try {
    const data = await $fetch<{ user: ManagedUser }>(`${config.public.apiBase}/auth/users/${route.params.id}`, { headers: headers() });
    applyUser(data.user);
...
const save = async () => {
  ...
  try {
    const data = await $fetch<{ user: ManagedUser }>(`${config.public.apiBase}/auth/users/${route.params.id}`, { method: 'PATCH', headers: headers(), body: { nickname: nickname.value, role: role.value, isActive: isActive.value } });
